				...
				[{Module, ModuleReal}] ->
					...
					case (catch apply(ModuleReal, handle, [Args])) of
						...
						Atom when is_atom(Atom) ->
							none;
						DataRtn ->
							case (catch mgee_packet:packet_encode_send(ClientSock, Module, Method, DataRtn) ) of
								{'EXIT', Info} -> 
									?ERROR_MSG("error when router packet_encode_send Module:~p, Method:~p, Info:~p", [Module, Method, Info]);
								_ -> ok
							end									
					end;
				_ -> 
					?DEBUG("undefined module ~p", [Module]), 
					ok
			end
	end.
